...
public class InternalInputBuffer extends AbstractInputBuffer<Socket> {
    ...
    public boolean parseRequestLine(boolean useAvailableDataOnly)
        ...
        while (!space) {
            ...
            if (buf[pos] == Constants.SP || buf[pos] == Constants.HT) {
                space = true;
                //需要注意的是，这里的setBytes并没有把bytes全复制到内存对象里，而是采用了ByteChunk标记的方式，通过标记buf的起止下标，
                //来找到对应的method,Tomcat中基本都是这种处理方式，不会直接复制到内存对象包括请求体，这样可以节省空间，毕竟每一个socket对应
                //一个8k的buf,处理多个请求，请求体数据也是很占内存的，而且涉及到字符集编码，所以Tomcat不会主动去解析，只会把字节保存在buf中，
                // 当应用中需要拿的时候request.getRequestMethod才会去解析,具体可以参考ByteChunk中的ToString方法，还有一层缓存，
                // 首次解析时底层最终还是数组复制，缓存是Tomcat通用的StringCache，这种思想是不是可以借鉴，运行时复制节省空间
    ...
    protected boolean fill(boolean block) throws IOException {
        ...
        if (parsingHeader) {
            ...
            // 如果还在解析请求头(包括请求行，请求头解析结束后置为false)，lastValid表示当前解析数据的下标位置，如果该位置等于buf的长度了，表示请求头的数据超过buf了。
            // 所以请求头的最大长度只能是buffer的size

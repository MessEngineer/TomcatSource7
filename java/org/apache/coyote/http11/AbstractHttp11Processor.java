...
public abstract class AbstractHttp11Processor<S> extends AbstractProcessor<S> {
    ...
    public SocketState process(SocketWrapper<S> socketWrapper)
        ...
        while (!getErrorState().isError() && keepAlive && !comet && !isAsync() &&
            ...
            try {
                ...
                // 解析请求行
                // 请求行读进tomcat的buffer缓冲区，最大8*1024 8KB可配置
    ...
    protected void prepareRequest() {
        ...
        //重写了equals方法，针对每个byte去比对，而不是将字节转换成String后再比对，Tomcat并不处理字符集转换
        ...
        //于是,这样就有了一个问题, 并不是所有的Server都会正确应答100-continue, 比如lighttpd,
        // 就会返回417 “Expectation Failed”, 则会造成逻辑出错
        //一般header里面不会设置这个expect属性
        if (http11) {
        ...
        //一般是有contentLength时，用IdentityInputFilter,请求体块传输的时候用的是chunkFilter,两个前置过滤器在应用层例如request.read()
        //请求体时候会触发doRead方法

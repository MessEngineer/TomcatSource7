    ...
    protected class Acceptor extends AbstractEndpoint.Acceptor {
        ...
        public void run() {
            ...
            while (running) {
                ...
                // 通常触发paused场景是，关闭Tomcat时，先会调用Connector.pause方法，停止接收新的请求，再关闭线程池，至于关闭线程池的
                // 处理策略，目前看是直接暴力shutdown，线程池关闭策略是否可配，有待考察
                while (paused && running) {
                ...
                try {
                    ...
                    //达到了最大连接数限制则等待,nio时候最大默认10000，bio就是最大线程数，默认200
                    //LimitLatch，基于AQS
                    ...
                    try {
                        ...
                        // 悟了！connector.stop()的时候，调用了unlockAccept();就是Tomcat创建了一个socket连接来唤醒这边，跳出循环！！！！
...
//            System.out.println("JioEndpoint关闭了socket");
    ...
    protected class SocketProcessor implements Runnable {
        ...
        public void run() {
            ...
            //由于一个长连接可能接收到多个请求，这边可能是同一个socket，sync
            synchronized (socket) {
                // 开始处理socket
                // Socket默认状态为OPEN
                try {
    ...
    public void bind() throws Exception {
        ...
        if (serverSocket == null) {
            try {
                if (getAddress() == null) {
                    //这里的backlog默认大小是100，指的是用于保存全连接状态的请求队列大小（Recv-Queue,每次accept方法会从这个队列的头部获取全连接状态的socket）
                    // ，其大小通过/proc/sys/net/core/somaxconn指定，在使用listen函数时，内核会根据传入的backlog参数与系统参数somaxconn，取二者的较小值。
    ...
    protected boolean processSocket(Socket socket) {
        ...
        try {
            ...
            //设置socket上剩余的长连接请求数,一个长连接默认的最大请求处理数100,可配置
            ...
            // 一个http请求对应一个线程？一个长连接默认最大处理100个请求
